...
exports.default = {
  signupWithEmail: function signupWithEmail(req, res, next) {
    ...
    !email ? next('You Must Provide Email.') : _user2.default.findOne({
      email: email
    }).then(function (user) {
      if (user) return next('403:Email is in use.');
      var origin = req.headers.origin;

      var tokenn = _services.token.generateTokenWithEmail(email);
      ...
      _services.SES.send(mailObj).then(function (email) {
  ...
  verifyEmailToken: function verifyEmailToken(req, res, next) {
    _services.token.verifyEmailToken(req.body.token, function (err, address) {
  ...
  signup: function signup(req, res, next) {
    ...
    _services.token.verifyEmailToken(req.params.token, function (err, address) {
      if (err || address !== email || !email || !password) return res.sendStatus(401);
      _user2.default.findOne({ email: email }).then(function (existingUser) {
        ...
        newUser.save().then(function (savedUser) {
          return res.send({
            token: _services.token.generateToken(savedUser),
  ...
  signin: function signin(req, res, next) {
    ...
    !email || !password ? next('You Must Provide Email And Password') : _user2.default.findOne({ email: email }).then(function (user) {
      if (!user) return next('404:User Is Not Found');
      user.comparedPassword(password, function (err, good) {
        err || !good ? next(err || '403:Password Is Incorrect') : res.send({ token: _services.token.generateToken(user), isAdmin: _config2.default.admin.list.indexOf(user.email) != -1 });
      });
    }).catch(next);
  },
